...
module.exports = {
  generate: (package, repository, templates, debug = false) => {
    const templateDir = path.resolve(__dirname, 'templates')
    const overrideTemplatesDir = templates ? path.resolve(process.cwd(), templates) : undefined;
    ...
    const registerPartial = (name, file, replacements) => {
      const dir = overrideTemplatesDir && existsSync(path.resolve(overrideTemplatesDir, file))
        ? overrideTemplatesDir
        : templateDir

      const data = readFileSync(path.resolve(dir, file))
